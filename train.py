...
def print_args(args, cfg):
    ...
    # Build a concise summary with only key training parameters
    cfg_summary = {
        "DATASET": {
            "NAME": cfg.DATASET.NAME,
            "NUM_SHOTS": cfg.DATASET.NUM_SHOTS,
            "SUBSAMPLE_CLASSES": cfg.DATASET.SUBSAMPLE_CLASSES,
        },
        "MODEL": {
    ...
    print(pformat(cfg_summary, indent=2, width=100))
    
    # Print trainer-specific config
    trainer_name = cfg.TRAINER.NAME.upper()
    trainer_specific = None
 
    if "RPLMED" in trainer_name and hasattr(cfg.TRAINER, "RPLMED"):
        trainer_specific = {"RPLMED": dict(cfg.TRAINER.RPLMED)}
    elif "COCOOP" in trainer_name and hasattr(cfg.TRAINER, "COCOOP"):
        trainer_specific = {"COCOOP": dict(cfg.TRAINER.COCOOP)}
    elif "KGCOOP" in trainer_name and hasattr(cfg.TRAINER, "KGCOOP"):
        trainer_specific = {"KGCOOP": dict(cfg.TRAINER.KGCOOP)}
    elif "BIOMEDCOOP" in trainer_name and hasattr(cfg.TRAINER, "BIOMEDCOOP"):
        trainer_specific = {"BIOMEDCOOP": dict(cfg.TRAINER.BIOMEDCOOP)}
    elif "PROGRAD" in trainer_name and hasattr(cfg.TRAINER, "PROGRAD"):
        trainer_specific = {"PROGRAD": dict(cfg.TRAINER.PROGRAD)}
    elif "COOP" in trainer_name and hasattr(cfg.TRAINER, "COOP"):
        trainer_specific = {"COOP": dict(cfg.TRAINER.COOP)}
    
    if trainer_specific:
        print("\n*********************")
        print("** Trainer Config **")
        print("*********************")
        print(pformat(trainer_specific, indent=2, width=100))
...
def extend_cfg(cfg):
    ...
    cfg.TRAINER.RPLMED = CN()
    cfg.TRAINER.RPLMED.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.RPLMED.CSC = False  # class-specific context
    cfg.TRAINER.RPLMED.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'
    cfg.TRAINER.RPLMED.N_CTX = 4  # number of context vectors
    cfg.TRAINER.RPLMED.PREC = "fp32"  # fp16, fp32, amp
    cfg.TRAINER.RPLMED.N_PROMPTS = 50
    cfg.TRAINER.RPLMED.KL_LAMBDA = 0.3
    cfg.TRAINER.RPLMED.L1_LAMBDA = 0.0  
    cfg.TRAINER.RPLMED.TAU = 0.0
