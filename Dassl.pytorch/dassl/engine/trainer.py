...
def _get_cfg_value(cfg_node, key, default):
    if not hasattr(cfg_node, key):
        return default
    return getattr(cfg_node, key)


def apply_test_corruption(input, cfg):
    """Apply optional test-time image degradation after normalization.

    The default is a no-op. This is used only for robustness evaluation and
    keeps training data augmentation unchanged.
    """
    corr_cfg = getattr(cfg.TEST, "CORRUPTION", None)
    corr_type = "none" if corr_cfg is None else str(_get_cfg_value(corr_cfg, "TYPE", "none")).lower()
    severity = 0.0 if corr_cfg is None else float(_get_cfg_value(corr_cfg, "SEVERITY", 0.0))
    if corr_type in {"", "none"} or severity <= 0:
        return input

    mean = torch.tensor(cfg.INPUT.PIXEL_MEAN, device=input.device, dtype=input.dtype).view(1, -1, 1, 1)
    std = torch.tensor(cfg.INPUT.PIXEL_STD, device=input.device, dtype=input.dtype).view(1, -1, 1, 1)
    x = torch.clamp(input * std + mean, 0.0, 1.0)

    if corr_type == "gaussian_noise":
        x = torch.clamp(x + torch.randn_like(x) * severity, 0.0, 1.0)
    elif corr_type == "gaussian_blur":
        kernel = 3 if severity <= 1.0 else 5
        pad = kernel // 2
        x = F.avg_pool2d(x, kernel_size=kernel, stride=1, padding=pad)
    elif corr_type == "brightness":
        x = torch.clamp(x + severity, 0.0, 1.0)
    elif corr_type == "contrast":
        channel_mean = x.mean(dim=(2, 3), keepdim=True)
        x = torch.clamp((x - channel_mean) * (1.0 + severity) + channel_mean, 0.0, 1.0)
    elif corr_type == "occlusion":
        b, c, h, w = x.shape
        ratio = min(max(severity, 0.0), 0.9)
        occ_h = max(1, int(h * ratio))
        occ_w = max(1, int(w * ratio))
        top = max(0, (h - occ_h) // 2)
        left = max(0, (w - occ_w) // 2)
        x = x.clone()
        x[:, :, top:top + occ_h, left:left + occ_w] = 0.0
    else:
        raise ValueError(f"Unknown TEST.CORRUPTION.TYPE: {corr_type}")
...
class SimpleTrainer(TrainerBase):
    ...
    def parse_batch_test(self, batch):
        ...
        input = apply_test_corruption(input, self.cfg)
