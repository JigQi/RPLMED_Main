...
class Classification(EvaluatorBase):
    ...
    def process(self, mo, gt):
        ...
        prob = torch.softmax(mo.detach(), dim=1)
        pred = prob.max(1)[1]
        ...
        self._y_prob.extend(prob.cpu().numpy().tolist())
    ...
    def evaluate(self):
        ...
        probs = np.asarray(self._y_prob, dtype=np.float64)
        labels = np.asarray(self._y_true, dtype=np.int64)
        eps = 1e-12
        true_probs = probs[np.arange(len(labels)), labels]
        nll = -np.log(np.clip(true_probs, eps, 1.0)).mean()
        one_hot = np.zeros_like(probs)
        one_hot[np.arange(len(labels)), labels] = 1.0
        brier = np.sum((probs - one_hot) ** 2, axis=1).mean()
